...
with graph.as_default():
    # Input data .For the training data,we use placeholder that will be fed at the  run time with a training minibatch
    ...
    # Prediction for the training Validation and test datasets
    train_prediction = tf.nn.softmax(logits)
    valid_prediction = tf.nn.softmax(
        tf.matmul(tf_valid_dataset,weights )+biases)
    test_predicition = tf.nn.softmax(tf.matmul(tf_test_dataset,weights)+biases)
